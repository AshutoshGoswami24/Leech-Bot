...
# Define a function to delete the DHT cache file
def delete_dht_cache():
    subprocess.run(["rm", "-f", "/root/.cache/aria2/dht.dat"])
...
async def show_download_progress(downloaded_bytes, total_file_size, message):
    ...
    await message.reply_text(f"Downloading: {percentage:.2f}%")
...
async def download_and_upload(url, message):
    ...
    proc = subprocess.Popen(
        command, bufsize=0, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
